...
wlroots / KDE / GNOME 任一后端在**连接中断**（compositor 重启、display 断开、扩展被禁用等）期间，对外的 D-Bus 契约**保持不变**：

- `GetFocus()` 返回两个**空串** `("","")` —— 表示「当前无法确认焦点」，绝不返回上一次的陈旧值（这正是迭代三根治的「常驻但说谎」问题）；
- 主循环 Dedup 保证断连期间只发射**恰好一次** `FocusChanged("","")`；
- 后端恢复（wlroots 自动重连 / KDE 自动重注册 / GNOME 扩展重新 enable）后，立即补推当前真实焦点，**无需重启 focusd**。

> 契约零变更：上述行为不改变任何方法 / 信号签名，消费方无需改动（R6 不在本迭代范围）。
