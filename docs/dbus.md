...
后端连接中断（如 wlroots compositor 重启重连、KDE KWin 重注册、GNOME 扩展临时失效）期间，serve 的契约语义保持确定：

- 期间 `GetFocus()` 立即返回 `ss "" ""`（无焦点，而非保留中断前的陈旧值）；
- 进入中断的瞬间会额外发射**恰好一次** `FocusChanged("", "")`，使订阅方状态同步为「无焦点」；
- 连接恢复后自动恢复正常上报（补推当前焦点，无需消费方重启）。

该方法 / 信号签名不变；行为变化仅体现在值语义上。
